...
/**
 * ContentSearch represents the model behind the search form about `common\modules\content\models\Content`.
 */
class ContentSearch extends Content
{
    ...
    */
    ...
    {
        $query = Content::find()->joinWith(['menu']);
        ...
        $query->andFilterWhere([
            '`content`.`id`' => trim($this->id),
            '`content`.`status`' => trim($this->status)
        ]);
        
        if($this->date_from != '' && $this->date_to != '') 
		{
			$query->andFilterWhere(['between', '`content`.`created_at`', trim(strtotime($this->date_from)), trim(strtotime($this->date_to))]);
		}

        $query->andFilterWhere(['like', '`content`.`title`', trim($this->title)]);
        $query->andFilterWhere(['like', '`menu`.`url`', trim($this->url)]);
